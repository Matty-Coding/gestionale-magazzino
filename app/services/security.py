...
class TokenService:
    ...
    def check_token(self, token: str, salt: str, expiration: timedelta) -> str | bool:
        ...
        try:
            email = self.serializer.loads(
                token, salt=salt, max_age=int(expiration.total_seconds())
            )
    ...
    def get_auth_token(self, email: str) -> str:
        ...
        """
        return self.generate_token(email=email, salt=self.autentication_salt)
    ...
    def check_auth_token(self, token: str, expiration=timedelta(hours=1)) -> str | bool:
        ...
        return self.check_token(
            token=token, salt=self.autentication_salt, expiration=expiration
        )
    ...
    def get_reset_password_token(self, email: str) -> str:
        ...
        """
        return self.generate_token(email=email, salt=self.reset_password_salt)

    def check_reset_password_token(
        self, token: str, expiration=timedelta(minutes=10)
    ) -> str | bool:
        """
        Verifica token per il reset password.
        """
        return self.check_token(
            token=token, salt=self.reset_password_salt, expiration=expiration
        )
