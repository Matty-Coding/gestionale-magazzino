...
def dashboard():
    match current_user.ruolo:
        case "FORNITORE":
            ...
            totale_pezzi = sum(
                fornitore_prodotto.prodotto.quantita
                for fornitore_prodotto in prodotti_fornitore
            )
...
def aggiungi_prodotto():
    ...
    if form_prodotto.validate():
        if prodottocrud.get_prodotto(data.get("codice")):
            return jsonify({"status": "error", "message": "Prodotto già presente"})

        prodottocrud.create_prodotto(
            codice=data.get("codice"),
            nome=data.get("nome"),
            descrizione=data.get("descrizione")
            or "Nessuna descrizione per questo prodotto",
...
@home_bp.route(
    "/admin/management/prodotto/modifica/<int:prodotto_id>", methods=["POST"]
)
@login_required
@admin_required
...
@home_bp.route(
    "/admin/management/prodotto/elimina/<int:prodotto_id>", methods=["DELETE"]
)
@login_required
@admin_required
...
def aggiungi_utente():
    ...
    if form_utente.validate():
        ...
        if user_obj:
            return jsonify({"status": "error", "message": "Utente già presente"})
...
def aggiungi_al_carrello(prodotto_id):
    ...
    return jsonify(
        {"status": "success", "html": html, "conta_prodotti": conta_prodotti}
    )
...
def genera_riepilogo_ordine(ordine):
    ...
    html_segments.append(
        f"<p style='font-size: 18px;'><strong>Totale complessivo: {ordine.totale}€</strong></p>"
    )
...
def test_stripe():
    try:
        intent = PaymentIntent.create(amount=1000, currency="eur")
...
def pagamento(ordine_id):
    ...
    session = Session.create(
        ...
        mode="payment",
        success_url=url_for(
            "home.pagamento_accettato", ordine_id=ordine_id, _external=True
        ),
        cancel_url=url_for(
            "home.pagamento_rifiutato", ordine_id=ordine_id, _external=True
        ),
...
def pagamento_accettato(ordine_id):
    ...
    flash(
        "Pagamento completato con successo! Stiamo elaborando il tuo ordine.", "success"
    )
    return redirect(url_for("home.miei_ordini"))
...
def stripe_webhook():
    ...
    try:
        event = Webhook.construct_event(
            payload=payload, sig_header=signature_header, secret=endpoint_secret
        )
