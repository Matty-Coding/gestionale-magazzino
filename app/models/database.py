...
class Categoria(db.Model):
    ...
    descrizione: Mapped[str] = mapped_column(
        Text, default="Nessuna descrizione per questa categoria."
    )
    attiva: Mapped[bool] = mapped_column(Boolean, default=True)

    # relazione con la tabella prodotti
    prodotti: Mapped[list["Prodotto"]] = relationship(
        "Prodotto", back_populates="categoria"
    )
...
class Prodotto(db.Model):
    ...
    nome: Mapped[str] = mapped_column(String(200), nullable=False)
    descrizione: Mapped[str] = mapped_column(
        Text, default="Nessuna descrizione per questo prodotto."
    )
    ...
    categoria: Mapped["Categoria"] = relationship(
        "Categoria", back_populates="prodotti"
    )

    # relazione con la tabella fornitori
    fornitori: Mapped[list["FornitoreProdotto"]] = relationship(
        "FornitoreProdotto", back_populates="prodotto", cascade="all, delete-orphan"
    )

    # relazione con la tabella ordini
    dettagli_ordine: Mapped[list["OrdineDettaglio"]] = relationship(
        "OrdineDettaglio", back_populates="prodotto"
    )
...
class Fornitore(db.Model):
    ...
    partita_iva: Mapped[str] = mapped_column(
        String(11), nullable=True, unique=True, index=True
    )
    ...
    # relazione con la tabella prodotti
    prodotti: Mapped[list["FornitoreProdotto"]] = relationship(
        "FornitoreProdotto", back_populates="fornitore", cascade="all, delete-orphan"
    )
    ...
    user: Mapped["User"] = relationship(
        "User", back_populates="fornitore", uselist=False
    )
...
class FornitoreProdotto(db.Model):
    ...
    fornitore_id: Mapped[int] = mapped_column(
        ForeignKey("fornitori.id"), primary_key=True
    )
    prodotto_id: Mapped[int] = mapped_column(
        ForeignKey("prodotti.id"), primary_key=True
    )
...
class User(db.Model, UserMixin):
    ...
    ordini: Mapped[list["Ordine"]] = relationship(
        "Ordine", back_populates="cliente", cascade="all, delete-orphan"
    )

    # relazione con la tabella fornitori
    fornitore: Mapped[Fornitore] = relationship(
        "Fornitore", back_populates="user", uselist=False, cascade="all, delete-orphan"
    )
...
class Ordine(db.Model):
    ...
    dettagli: Mapped[list["OrdineDettaglio"]] = relationship(
        "OrdineDettaglio", back_populates="ordine", cascade="all, delete-orphan"
    )
...
class OrdineDettaglio(db.Model):
    ...
    prodotto: Mapped["Prodotto"] = relationship(
        "Prodotto", back_populates="dettagli_ordine"
    )
